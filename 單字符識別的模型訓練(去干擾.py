...
class TrainingLogger:
    def __init__(self):
        self.log_file = f"denoised_training_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
...
class DenoisedCharDataset(Dataset):
...
# 修改後的訓練函數
def train_denoised_char(model, train_loader, criterion, optimizer, device, update_every=1, scheduler=None):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
...
# 驗證函數
def validate_denoised_char(model, val_loader, criterion, device):
    model.eval()
    running_loss = 0.0
    correct = 0
    total = 0
...
def add_gradient_monitoring(model, logger, epoch_interval=5):
    ...
    original_train = train_denoised_char
...
def main():
    ...
    # 配置參數
    denoised_char_dir = r"C:\Users\C14511\Desktop\captcha\CAPTCHA\denoised"  # 請修改為您的去干擾資料集路徑
    batch_size = 16  # 可增加批量大小，因為圖像已去干擾
    denoised_char_epochs = 300  # 降低輪數
    learning_rate = 0.0005  # 調整學習率
    early_stopping_patience = 15  # 提前停止耐心度
    checkpoint_interval = 10  # 檢查點間隔
    update_every = 2  # 梯度累積步數
    
    # 定義資料轉換（對去干擾圖像可減少擾動）
    transform = transforms.Compose([
        transforms.Resize((69, 37)),
        transforms.RandomRotation(3),  # 減少旋轉角度
        transforms.RandomAffine(degrees=0, translate=(0.05, 0.05), scale=(0.95, 1.05), shear=3),  # 減少仿射變換
        transforms.ColorJitter(brightness=0.2, contrast=0.2),  # 減少顏色抖動
        transforms.ToTensor(),
    ...
    logger.log("開始訓練去干擾字元模型...")
    
    try:
        # 創建去干擾字符數據集
        denoised_char_train_dataset = DenoisedCharDataset(denoised_char_dir, transform=transform, split='train')
        denoised_char_val_dataset = DenoisedCharDataset(denoised_char_dir, transform=val_transform, split='val')
        
        # 顯示樣本預處理效果
        logger.log("正在生成範例預處理圖片...")
        sample_img_path = preprocess_and_show_sample(denoised_char_train_dataset)
        logger.log(f"範例圖片已保存至 {sample_img_path}")
        
        # 記錄數據集信息
        logger.log(f"數據集信息:")
        logger.log(f"- 訓練集大小: {len(denoised_char_train_dataset)}")
        logger.log(f"- 驗證集大小: {len(denoised_char_val_dataset)}")
        logger.log(f"- 字符類別數: {len(denoised_char_train_dataset.char_to_idx)}")
        logger.log(f"- 字符集: {denoised_char_train_dataset.char_to_idx}")
        
        denoised_char_train_loader = DataLoader(
            denoised_char_train_dataset,
        ...
        denoised_char_val_loader = DataLoader(
            denoised_char_val_dataset,
            batch_size=batch_size,
            shuffle=False,
            num_workers=0,
            pin_memory=False
        ...
        # 初始化去干擾字符模型
        logger.log("初始化去干擾字符模型...")
        denoised_char_model = SingleCharCNN(
            num_classes=len(denoised_char_train_dataset.char_to_idx),
            image_height=69,
            image_width=37,
            dropout_rate=0.25  # 減少dropout率
        ).to(device)
        logger.log(f"模型參數數量: {count_parameters(denoised_char_model):,}")
        
        # 打印初始化後的模型結構和權重
        logger.log("\n===== 初始化模型結構和權重 =====")
        print_model_structure_and_weights(denoised_char_model, logger)
        visualize_weights(denoised_char_model, 'denoised_initial_weights.png')
        logger.log(f"初始權重視覺化已保存至 denoised_initial_weights.png")

        # 使用標準交叉熵損失函數
        criterion = nn.CrossEntropyLoss()
        
        # 使用帶有權重衰減的AdamW優化器
        optimizer = optim.AdamW(denoised_char_model.parameters(), lr=learning_rate, weight_decay=5e-6)
        
        # 使用餘弦衰減學習率
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=denoised_char_epochs)
        
        best_denoised_char_accuracy = 0
        no_improve_count = 0

        # 添加梯度監控 (每10個epoch記錄一次)
        logger.log("正在設置梯度監控...")
        train_denoised_char_with_monitoring = add_gradient_monitoring(denoised_char_model, logger, epoch_interval=20)

        logger.log("開始訓練循環...")
        for epoch in range(denoised_char_epochs):
            # 手動執行垃圾回收
            gc.collect()
            
            logger.log(f"\n去干擾字元模型訓練週期 {epoch+1}/{denoised_char_epochs}")
            
            # 使用梯度累積進行訓練
            train_loss, train_acc = train_denoised_char_with_monitoring(
                denoised_char_model, 
                denoised_char_train_loader, 
                criterion, 
                optimizer, 
                device, 
            ...
            val_loss, val_acc = validate_denoised_char(
                denoised_char_model, 
                denoised_char_val_loader, 
                criterion, 
                device
            )
            ...
            # 保存檢查點
            if val_acc > best_denoised_char_accuracy:
                best_denoised_char_accuracy = val_acc
                no_improve_count = 0
                save_checkpoint(
                    epoch, 
                    denoised_char_model, 
                    optimizer, 
                    best_denoised_char_accuracy, 
                    denoised_char_train_dataset.char_to_idx, 
                    list(denoised_char_train_dataset.char_to_idx.keys()),
                    'best_denoised_char_model.pth' # 添加 filename 參數
                )
                logger.log(f"已儲存新的最佳去干擾字符模型，準確率: {best_denoised_char_accuracy:.2f}%")
                
                # 在保存最佳模型時記錄當前權重
                logger.log("\n===== 最佳模型的權重信息 =====")
                print_model_structure_and_weights(denoised_char_model, logger)
                visualize_weights(denoised_char_model, 'best_denoised_model_weights.png')
                logger.log(f"最佳模型權重視覺化已保存至 best_denoised_model_weights.png")
            else:
                no_improve_count += 1
            ...
            if (epoch + 1) % checkpoint_interval == 0:
                save_checkpoint(
                    ...
                    denoised_char_train_dataset.char_to_idx, 
                    list(denoised_char_train_dataset.char_to_idx.keys()),
                    f'denoised_char_model_epoch_{epoch+1}.pth'
                )
        ...
        logger.log(f"\n去干擾字元模型訓練完成！最佳驗證準確率: {best_denoised_char_accuracy:.2f}%")
        
        # 再次查看最終模型的權重
        logger.log("\n===== 最終模型結構和權重 =====")
        print_model_structure_and_weights(denoised_char_model, logger)
        visualize_weights(denoised_char_model, 'final_denoised_weights.png')
        logger.log(f"最終權重視覺化已保存至 final_denoised_weights.png")
        ...
        errors = analyze_errors(
            denoised_char_model, 
            denoised_char_val_loader, 
            {v: k for k, v in denoised_char_train_dataset.char_to_idx.items()}, 
            device, 
            num_samples=10
        )
        ...
        if errors:
            ...
            plt.tight_layout()
            plt.savefig('denoised_error_analysis.png')
            logger.log("錯誤分析完成並保存到denoised_error_analysis.png")
        
        # 在訓練結束後，添加以下代碼
        gradient_vis_path = denoised_char_model.visualize_gradients('denoised_gradient_statistics.png')
        logger.log(f"梯度統計視覺化已保存至: {gradient_vis_path}")
        denoised_char_model.remove_gradient_hooks()
