...
def train(model, train_loader, criterion, optimizer, scheduler, device, epoch, total_epochs, logger, use_mixup=True, gradient_monitor=None):
    ...
    for images, labels in progress_bar:
        ...
        if gradient_monitor:
            gradient_monitor.step() # Trigger gradient logging
...
def main():
    config = {
        'train_dir': r"C:\Users\C14511\Desktop\captcha\CAPTCHA\000",        # 訓練資料集目錄
        'test_dir': r"C:\Users\C14511\Desktop\captcha\CAPTCHA\training",    # 測試資料集目錄
        'pretrained_model_original': r"C:\Users\C14511\captcha\best_single_char_model.pth", # 預訓練模型路徑 (可選)
    ...
    try:
        ...
        if 'pretrained_model_original' in config and config['pretrained_model_original']:
            logger.log(f"嘗試從 {config['pretrained_model_original']} 載入預訓練權重...")
            success, _ = load_pretrained_weights(model, config['pretrained_model_original'], logger)
            if not success:
                logger.log("載入預訓練模型失敗，使用隨機初始化的權重")
                logger.summary("使用隨機初始化的權重 (無法載入預訓練模型)")
            else:
                logger.summary(f"成功載入預訓練模型: {config['pretrained_model_original']}")
        ...
        for epoch in range(config['num_epochs']):
            ...
            if test_char_acc > best_char_accuracy or test_string_acc > best_string_accuracy:
                best_string_accuracy = max(best_string_accuracy, test_string_acc)
                best_char_accuracy = max(best_char_accuracy, test_char_acc)
                improved = True
                checkpoint = {
                    'epoch': epoch,
                ...
                torch.save(checkpoint, os.path.join('models', 'orig_img_best_model.pth'))
                logger.log(f"已儲存新的最佳模型，字串準確率: {best_string_accuracy:.2f}%，字元準確率: {best_char_accuracy:.2f}%")
                logger.summary(f"新的最佳模型: 字串準確率={best_string_accuracy:.2f}%, 字元準確率={best_char_accuracy:.2f}%")
                no_improve_count = 0
                ...
                # 在此處添加checkpoint_string_accuracy和checkpoint_char_accuracy的定義
                checkpoint_string_accuracy = test_string_acc  # 定義用於儲存模型的字串準確率
                checkpoint_char_accuracy = test_char_acc  # 定義用於儲存模型的字元準確率
                
                model_type = config['model_type']  # 使用配置中的模型類型標識符
                checkpoint = {
                    'epoch': epoch,
                    'model_state_dict': model.state_dict(),
                    'optimizer_state_dict': optimizer.state_dict(),
                ...
                torch.save(checkpoint, os.path.join('models', f'{model_type}_best_model.pth'))
                ...
                if (epoch + 1) % config['checkpoint_interval'] == 0:
                    ...
                    torch.save(checkpoint, os.path.join('models', f'{model_type}_checkpoint_e{epoch+1}.pth'))
        ...
        logger.log(f"訓練完成！最佳測試準確率: {best_string_accuracy:.2f}%")
        logger.summary(f"訓練完成！最佳字串準確率: {best_string_accuracy:.2f}%, 最佳字元準確率: {best_char_accuracy:.2f}%")
