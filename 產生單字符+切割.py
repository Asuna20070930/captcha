...
# 按字體區分字符集
arial_characters = ['8', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'm', 'p', 'r', 'w', 'x', 'y']
ebrima_characters = ['2', '3', '4', '5', '6', '7', 'a', 'k']
...
class GrayCaptchaGenerator:
    ...
    def generate_captcha(self, text=None, length=6):
        ...
        for i, char in enumerate(display_text):
            ...
            # 保存處理後的字符圖像和其位置
            char_images.append((char_img_processed, x, char, char_width))
        ...
        for char_img, x_pos, _, _ in char_images:
        ...
        return final_image, captcha_text, char_images

    def batch_generate(self, count, output_dir="captchas"):
        """批量生成CAPTCHA圖像並保存，同時將每個字符單獨切割並儲存"""
        # 確保輸出目錄存在
        ...
        with open(csv_path, "w") as f:
            ...
            for i in range(count):
                ...
                # 生成圖像
                img, text, char_images = self.generate_captcha()
                ...
                # 切割並保存每個字符
                for j, (char_img, x_pos, char, char_width) in enumerate(char_images):
                    # 為每個字符創建子目錄
                    ...
                    # 縮放為最終尺寸後的字符位置和寬度
                    scale_factor = self.final_width / self.initial_width
                    x_pos_scaled = int(x_pos * scale_factor)
                    char_width_scaled = int(char_width * scale_factor)
                    
                    # 為了確保完整切割字符，在寬度上添加適當的邊距
                    margin = 5  # 左右各添加5像素邊距
                    left = max(0, x_pos_scaled - margin)
                    right = min(self.final_width, x_pos_scaled + char_width_scaled + margin)
                    
                    # 從完整的縮放後圖像中裁剪出單個字符
                    char_crop = img.crop((left, 0, right, self.final_height))
                    
                    # 保存裁剪後的字符圖像
                    char_crop_filename = f"{text}_{j}_{char}.png"
                    char_crop_filepath = os.path.join(char_dir, char_crop_filename)
                    char_crop.save(char_crop_filepath)
        
        print(f"\n批量生成完成！共生成了{count}張CAPTCHA圖像。")
        print(f"圖像和標籤已保存到 {os.path.abspath(output_dir)} 目錄。")
        print(f"每個字符已分別切割並保存到相應的子目錄中。")
...
if __name__ == "__main__":
    try:
        ...
        # 批量生成CAPTCHA圖像
        num_images = 3000  # 要生成的圖像數量
        output_dir = r"C:\Users\C14511\Desktop\captcha\CAPTCHA\0"  # 輸出目錄
        ...
        for i in range(4):
            img, text, _ = captcha_generator.generate_captcha()
